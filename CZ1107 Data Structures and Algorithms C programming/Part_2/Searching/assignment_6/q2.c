...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    //find the hash first of all 
    //still yet to edit 
    //do we need to check if it is a full list, 
    //previous node not done properly
    
    int index=0;
    ListNode *keyexist, *new_node, *temp;
    index = Hash(key, Q1Ptr->hSize);
    keyexist = HashSearch(*Q1Ptr, key);
    if (keyexist != NULL){
        return 0;
    }else{
        if (Q1Ptr->Table[index].head==NULL){
            Q1Ptr->Table[index].head = (ListNode*)malloc(sizeof(ListNode));
            Q1Ptr->Table[index].head->key = key;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    //Write your code here
    int index=0;
    ListNode *keyexist=NULL, *prev, *next;
    index = Hash(key, Q1Ptr->hSize);
    keyexist = HashSearch(*Q1Ptr, key);
    if (keyexist == NULL){
        return 0;
        ...
        //here goes the actual deleting
        if (Q1Ptr->Table[index].head==keyexist){
            prev = Q1Ptr->Table[index].head;
            Q1Ptr->Table[index].head = Q1Ptr->Table[index].head->next;
            Q1Ptr->Table[index].head->pre = NULL;
            free(prev);

        }else if(keyexist->next == NULL) {
            keyexist->pre->next= NULL;
            free(keyexist);
        }
        else{
            prev = keyexist->pre;
            next = keyexist->next;
            prev->next = keyexist->next;
            next->pre = prev;
            free(keyexist);
