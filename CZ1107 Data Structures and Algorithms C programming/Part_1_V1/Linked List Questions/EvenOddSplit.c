...
int split(ListNode *cur, ListNode **ptrEvenList,ListNode **ptrOddList)
{
    ...
    printf("%d", temp->item);
    while(temp){
        insertNode(ptrEvenList, index_even, temp->item);
        temp=temp->next;
	    index_even++;
        if (temp){ //here this was important!
            insertNode(ptrOddList, index_odd, temp->item);
            temp=temp->next;
            index_odd++;
        }
        //at every step ask if what you are entering in null or not 
    }
