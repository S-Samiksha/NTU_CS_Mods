...
void printList2(LinkedList ll){
    if(ll.head != NULL){
        ...
        printf("Current List has %d elements: ",ll.size); // you have to print the size out 

        while (cur != NULL){ //this is the same as question 1 
            printf("%d ", cur->item);
            cur = cur->next;
        }
        printf("\n");
...
ListNode* findNode2(LinkedList ll, int index) //returning a pointer 
{
   if(ll.head != NULL){
        ListNode *cur = ll.head; //make a current pointer for the head 
     ...
     return NULL; //this is if the list is empty 
}
...
int removeNode2(LinkedList *ll,int index)
{
    ...
    //find the previous, current and next nodes in the linked list
    ListNode *pre, *next, *cur;
    pre = findNode2(*ll, index-1);
    cur = findNode2(*ll, index);
    next = findNode2(*ll, index+1);
    if (cur == NULL){ //remember this condition!!
        ...
        pre->next = next;
        cur->next = NULL; 
        //remeber to reduce index by one 
        ll->size--;
