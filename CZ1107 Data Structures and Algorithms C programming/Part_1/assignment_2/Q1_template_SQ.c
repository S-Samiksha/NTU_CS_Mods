...
void removeOddValues(Queue *q)
{
	...
	temp_list.head = NULL; //MUST initialize the linked list head 
	temp_list.size = 0; //MUST initialize the linked list size
	int temp_val, index=0;
	if (!isEmptyQueue(q)){
		while(!isEmptyQueue(q)){
			temp_val = dequeue(q); //returns the item not the node 
			if(temp_val%2 == 0){
				insertNode(&temp_list, index, temp_val);
				index++;
			}
		}
		createQueueFromLinkedList(&temp_list, q);		
	}
}
