...
void removeEvenValues(Stack *s)
{
	// add your code here
	Stack temp_stack;
	temp_stack.ll.head = NULL; //MUST initialize the linked list head 
	temp_stack.ll.size = 0; //MUST initialize the linked list size
	int temp_val=0;
	if (!isEmptyStack(s)){
		while(!isEmptyStack(s)){
			temp_val = pop(s); //returns the item not the node 
			if(temp_val%2 == 1){
				push(&temp_stack, temp_val);
			}
		}
		removeAllItemsFromStack(s);
		while(!isEmptyStack(&temp_stack)){
			temp_val = pop(&temp_stack);
			push(s, temp_val);
		}		
	}
}
