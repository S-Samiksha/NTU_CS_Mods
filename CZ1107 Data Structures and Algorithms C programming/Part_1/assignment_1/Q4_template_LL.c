...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	resultFrontList->head->item = temp->item;
	resultFrontList->head->next = temp->next;
	//Always conduct a check to see if the list is empty in the first place 
    if (ll->size > 2){
		while (count < size){
			temp = temp->next;
			count++;
