...
int VpnToPhyPage(int vpn)
{
  ...
  //return the right values despite segmentation fault 
  ...
  for (j=0;j<NumPhysPages;j++){
   if (memoryTable[j].valid==TRUE && memoryTable[j].pid==currentThread->pid && memoryTable[j].vPage==vpn){ i = j; 
...
int lruAlgorithm(void)
{
  ...
  int phyPage, lastused = 9999;
  int i=0;
  for (i=0;i<NumPhysPages;i++){
    if (!memoryTable[i].valid){ return i;}
    else if (memoryTable[i].lastUsed < lastused){
