...
{
    /* add your code here */
	//you must initialize the tail pointer 
	//must also check if the k is more than the size of the list /if k is not in the list 
	if (q->ll.size > 1){
		Stack temp;
		Queue tempq;
		int size=0, i=0, item =0;
		size = q->ll.size;
		temp.ll.head = NULL;
		temp.ll.size =0;
		tempq.ll.head = NULL;
		tempq.ll.size =0;
		while (q->ll.head->item != k){
			item = dequeue(q);
			enqueue(&tempq, item);
			push(&temp, item);
		}
