...
void sortStack(Stack *s)
{
    /* add your code here */
	Stack temp;
	int curr_item = 0, out_item =0;
	temp.ll.head = NULL; //initialize temp
	temp.ll.size = 0;
	...
	if (s->ll.size > 1){
		while(!isEmptyStack(s)){
			push(&temp, pop(s));
		} //transfer all the contents of s into temp 


		while(!isEmptyStack(&temp)){
			if (peek(&temp) < curr_item){
				curr_item = pop(&temp);
				push(s,curr_item);
				printf("%d \n", curr_item);
			} else if (peek(&temp) > curr_item){
				out_item = pop(s); //store the value in another variable first 
