...
int moveMaxToFront(ListNode **ptrHead)
{
	...
	//in this answer, you are storing the node previous to the maximum node
int moveMaxToFront(ListNode **ptrHead){
     ListNode *pre, *cur;
     ...
     if (*ptrHead == NULL) 
          return -1; // Empty list
     if ( (*ptrHead)->next == NULL ) // Only one node in the list
          return 0;
     cur = (*ptrHead);
	...
	cur = cur->next;
     ...
     cur = preMax->next; //since you know the previous to max --> you can know the maximum 
